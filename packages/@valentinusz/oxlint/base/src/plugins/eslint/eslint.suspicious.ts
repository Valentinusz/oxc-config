...
export const eslintSuspicious = {
  rules: {
    ...
    // 🐛 Bug prevention. Can lead to unexpected bugs.
    ...
    // 🔒 Security. Eval and similar methods have bad performance and lead to security risks.
    'eslint/no-implied-eval': 'error',

    // ✅ eslint/no-new
    // Disallow the use of the new operator when not part of an assignment or comparison.
    // 🐛 Bug prevention. Not storing and using the newly created object is often a bug.
    'eslint/no-new': 'error',
    ...
    // ⛓️ Too restrictive. It is useful to indicate that a variable is purposefully unused by prefixing with an
    // underscore.

    // ✅ eslint/no-unexpected-multiline
    // 🐛 Bug prevention. Disallow confusing multiline expressions
    'eslint/no-unexpected-multiline': 'error',

    // ✅ eslint/no-unmodified-loop-condition
    // Disallow references in loop conditions that are never modified within the loop.
    // 🐛 Bug prevention. This can lead to infinite loops.
    ...
    // ➡️ Handled by eslint/prefer-template
    ...
    // property.
    // 🐛 Bug prevention. Re-throwing an error without preserving the original error loses important debugging
    // information.
