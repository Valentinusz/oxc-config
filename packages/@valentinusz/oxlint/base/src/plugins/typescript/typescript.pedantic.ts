...
export const typescriptPedantic = {
  rules: {
    ...
    // 🏆 Best practice. It is useful to be warned of using deprecated code.
    ...
    // 🐛 Bug prevention. Enums themselves are discouraged, but this can help spot errors if you are using them.
    ...
    // 🐛 Bug prevention. Any is already forbidden, but library functions might still return any which you can then
    // forward as parameters without realizing resulting in losing type safety.
    'typescript/no-unsafe-argument': 'error',

    // ✅ typescript/no-unsafe-assignment
    // 💭 Type aware.
    // This rule disallows assigning a value with type any to variables and properties.
    // 🐛 Bug prevention. The `any` type in TypeScript is an escape hatch from the type system.
    ...
    // 🏆 Best practice. Function type syntax is preferred.
    ...
    // 🏆 Best practice. Feel free to disable if a library relies on using throw with a function call
    // (e.g., TanStack Router).
    'typescript/only-throw-error': 'error',

    // ✅ typescript/prefer-enum-initializers
    // Require each enum member value to be explicitly initialized.
    // 🐛 Bug prevention. Enums are discouraged, but implicit values can cause bugs if enums are modified over time.
    ...
    // 🏆 Best practice. `@ts-expect-error` is stricter.
    ...
    // 🐛 Bug prevention. Async functions that don't use await are usually a mistake.
    ...
    // 🏆 Best practice. Require await when it would affect error handling.
    'typescript/return-await': ['error', 'in-try-catch'],

    // ✅ typescript/strict-boolean-expressions
    // 💭 Type aware.
    // Disallow certain types in boolean expressions.
    // 🐛 Bug prevention. It is better to explicitly convert values to booleans in conditionals.
    'typescript/strict-boolean-expressions': 'error',

    // ✅ typescript/strict-void-return
    // 💭 Type aware.
    // Disallow returning non-void values where a `void` return is expected.
    // 🐛 Bug prevention. Returning values from void contexts can hide logic errors and make callback APIs behave
    // unexpectedly.
